...
// MINIMAL circuit: proves the final hardened step from the m/44'/313'/n'/0' parent node.
...
(async () => {
  ...
  const okTamperNew = await snarkjs.groth16.verify(vk, tNew, proof);
  const tDom = publicSignals.slice(); tDom[2] = (BigInt(tDom[2]) ^ 1n).toString();
  const okTamperDom = await snarkjs.groth16.verify(vk, tDom, proof);
