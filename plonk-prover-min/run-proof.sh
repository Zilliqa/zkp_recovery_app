...
SNARKJS="node --max-old-space-size=16384 node_modules/snarkjs/build/cli.cjs"
echo "proving (PLONK, minimal circuit ~427k constraints)..."
$SNARKJS plonk prove circuit_final.zkey circuit.wtns proof.json public.json >/dev/null
if $SNARKJS plonk verify vk.json public.json proof.json 2>&1 | grep -q OK; then
