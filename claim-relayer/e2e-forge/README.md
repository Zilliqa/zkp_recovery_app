...
- `gen_calldata.cjs` — regenerate `claim.json` (only if you change the circuit/key).
...
```bash
ZKEY=/path/to/final.zkey VK=/path/to/vk.json node gen_calldata.cjs
```
...
- `newAddr` and `domain` in the proof are set at prove time; the committed proof uses `domain=32769`
  and a fixed test `newAddr` — see `gen_calldata.cjs`.
